...
class TelnetClient():
    ...
    def download_logs(self, host_ip, localpath):
        """
        从远程设备下载 /var/log 目录下的日志文件
        """
        ftp = self.ftpConnect(host_ip, 21)
        remote_log_path = "/var/log/"
        local_log_path = os.path.join(localpath, "logs", host_ip.replace('.', '_'))
        ...
        try:
            ...
            for filename in filenames:
                local_file = os.path.join(local_log_path, filename)
                with open(local_file, 'wb') as fp:
                    ftp.retrbinary(f'RETR {filename}', fp.write)
                logging.info(f"{filename} 下载成功")
        except Exception as e:
            logging.error(f"下载日志文件时出错: {e}")
        finally:
            ftp.quit()
...
def main():
    ...
    def download_logs():
        telnet_client = TelnetClient('root', 'cvcam')
        ips_list = telnet_client.get_ip()
        if not ips_list:
            messagebox.showerror("错误", "没有读取到任何设备 IP，请检查 ips.txt 文件。")
            return

        for host_ip in ips_list:
            logging.info(f"[{host_ip}] 开始下载日志...")
            telnet_client.download_logs(host_ip, localpath)
    ...
    # 添加“下载日志”按钮
    download_btn = tk.Button(frame, text="下载日志", command=download_logs, width=30)
